        ...
        <main className="flex-1 max-w-5xl mx-auto w-full px-4 py-10 space-y-6">
          ...
          <section className="relative overflow-hidden rounded-2xl border border-border bg-card p-8 animate-fade-in-up">
            <div className="pointer-events-none absolute -top-16 -right-16 h-56 w-56 rounded-full blur-3xl"
                 style={{ background: 'color-mix(in srgb, var(--tag-general) 12%, transparent)' }} />
            <div className="relative flex items-center gap-5">
              <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl tag-general shadow-lg">
                <Database className="h-8 w-8" />
              </div>
              <div>
                <h1 className="text-2xl sm:text-3xl font-extrabold text-foreground">Databases</h1>
                <p className="text-muted text-sm mt-1 max-w-xl leading-relaxed">
                  Redis caching, MongoDB optimization, SQL schemas, data modeling, and storage architecture patterns.
                </p>
          ...
          {activeTag && (
            <div className="flex items-center justify-between bg-card border border-border px-4 py-2.5 rounded-xl animate-fade-in-up">
              <div className="flex items-center gap-2">
                ...
                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-semibold tag-general text-primary-foreground">
                  {activeTag}
                  <Link
                    href="/coding/databases"
          ...
          {posts.length === 0 ? (
            <div className="rounded-xl border border-border bg-card p-12 text-center">
              ...
              <div className="flex justify-center gap-4 mt-3">
                ...
                {activeTag && (
                  <Link href="/coding/databases" className="text-sm text-primary hover:underline">
                    Clear tag filter
                  </Link>
                )}
              </div>
  ...
  return (
    <Suspense fallback={
      <div className="flex flex-col min-h-screen bg-background text-foreground">
        <Navbar showSearch />
        <main className="flex-1 max-w-5xl mx-auto w-full px-4 py-10 flex items-center justify-center">
          <div className="text-muted text-sm font-semibold animate-pulse">Loading databases page...</div>
        </main>
        <Footer />
      </div>
    }>
